...
def gather_data():
    ...
    # Make sure that Selenium can get console.log messages that are generated by JavaScript later in this code
    dc = DC.CHROME
    dc["goog:loggingPrefs"] = {'browser' : 'ALL'}
    o = Options()
    driver = webdriver.Chrome(o)

    # Create a few dictionaries to store everything
...
def get_pages(driver, ms_dict):
    ...
    for page in pages:
        ...
        if id[-1] == '0':
            print(f'\tGot page {page_num} in {ms_id}')
            ...
            if contents != "No Index Content" and contents != '':
                # For now just scan through all these pages, and note that the transcription should be grabbed
                pages_to_trans.append((page_num, contents))

    # Get the transcriptions for all the pages saved in pages_to_trans
    for page_num, contents in pages_to_trans:
        try:
...
def get_trans(driver, ms_id, page_num, contents, ms_dict):
    ...
    # Load the transcript
    got500, blank, log = load_page(driver, ms_id, page_num, apoc_flag)
    ...
    if got500 or (blank and apoc_flag != ""):
        got500, blank, log = load_page(driver, ms_id, page_num, "")
    
    print(f'\tGrabbing transcription from {ms_id} at page {page_num} ({contents})')
    ...
    for regexed in log:
        ...
        vv = re.findall(r'@@\d?\w+\.?\d+\.\d+@@', regexed)
        idxx = []
        tracker = 0
        for v in vv:
            # If there are multiple references to a verse in a single transcription, we want to make sure that we get them in order. This does that
            part = regexed[tracker:]
            loc = part.find(v)
            idxx.append(loc + tracker)
            tracker += loc + len(v)
        ...
        for v in vv:
            ...
            if re.search('\.0', ref) is None:
                ...
                if re.search(r', *$', text) is None:
                    ...
                    # Edge case to accurately create bcv when there is no period between the book and chapter
                    if re.search(r'[A-Za-z]\d', bcv[0]) is not None:
                        cv = re.split(r'([A-Za-z]+)(\d+)', bcv[0])
                        bcv = (cv[1], cv[2], bcv[1])
                    
                    # Remove non-Greek characters
                    text = re.sub(r'[^ςερτυθιοπασδφγηξκλζχψωβνμ \[\]]', '', text)
...
# Load a page with an INTF transcript and do some initial parsing before returning the partially formatted output
def load_page(driver, ms_id, page_num, apoc_flag):
    ...
    # Create an AJAX request that get's the page and removes unwanted extraneous parts with JavaScript. Output is in the console
    driver.execute_script(f'''
        let xhr = new XMLHttpRequest()
        xhr.open("GET", "{page}", false)
        xhr.send()
        tdocument = new DOMParser().parseFromString(xhr.response, "text/html")

        //remove the line numbers, column numbers, folio numbers, and lacuna indicators that contain numbers
        tdocument.querySelectorAll(".linenumber").forEach(e => e.remove());
        tdocument.querySelectorAll(".lac").forEach(e => e.remove());
        tdocument.querySelectorAll(".columnBreak").forEach(e => e.remove());
        tdocument.querySelectorAll(".folioBreak").forEach(e => e.remove());

        //put supplied text in brackets and solve the issue of line breaks in the middle of words
        tdocument.querySelectorAll(".supplied").forEach(e => {{e.innerText = "[" + e.innerText + "]"}});
        tdocument.querySelectorAll(".nobreak").forEach(e => e.innerText = "~~");

        //remove corrections (ideally put in parens, but not yet)
        tdocument.querySelectorAll(".readings").forEach(e => {{
            e.remove()
        ...
        tdocument.querySelectorAll(".note").forEach(e => e.remove());
        tdocument.querySelectorAll(".note-marker").forEach(e => e.remove());
        tdocument.querySelectorAll(".app").forEach(e => e.remove());
        tdocument.querySelectorAll(".app-rdgs").forEach(e => e.remove());
        tdocument.querySelectorAll(".verse_number").forEach(e => e.remove());
        
        //Put the full verse reference before each verse using the DOM
        tdocument.querySelectorAll(".verse_marker").forEach(e => {{
            try {{
            ...
            padChk = e.dataset.lectid === undefined ? osisid.match(/\.?\d+/g) : lectid.match(/\.?\d+/g);
            zeroPad = padChk === null ? ".0.0@@" : padChk.length === 1 ? ".0@@" : "@@";
            e.innerText = e.dataset.lectid === undefined ? "@@" + osisid + zeroPad : "@@" + lectid + zeroPad;
            e.style.display = 'Block';
        }});
        
        for (let tran of tdocument.getElementsByClassName('msstrans')) {{
            console.log(tran.innerText)
        }}
    ''')

    # Capture console output from the browser and prepare it to be used
    log = driver.get_log("browser")
    got500 = False
    blank = False
    txt_obj = []
    for item in log:
        # Mark 500 internal server errors so that the page can (maybe) be retrieved with a different link
        if item['level'] == 'SEVERE':
            got500 = item['message'].__contains__('500')

        # Capture the normal output of console.log
        elif item['level'] == 'INFO':
            # If blank, mark it
            if item['message'].__contains__('No Transcription Available'):
                blank = True
            else:
                raw_txt = item['message'][item['message'].find('"')+1 : -1]
                txt = re.sub(r'\\n', r'\n', raw_txt)
                txt_obj.append(txt)

    return got500, blank, txt_obj
